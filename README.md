...
Obviously with a constant signal generated by the board itself the FFT is computed only once at the beginning so the frequency is adapted only once, in a more realistic scenario where the signal comes from the reading of a sensor input the code can be modified to adapt the sampling frequeuncy when a certain condition is met.
...
The task `lorawan_transmission_task` checks the current communication method, and if LoRa is selected, it prepares the data for transmission and sends it via the `LoRaWAN.send()` function found in the `loop()` function that. This ensures that the data is sent to the TTN network in a reliable and structured way.
